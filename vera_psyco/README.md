...
Candidate onboarding agent. Conducts a structured roleplay conversation to produce `who_are_u.md`
under the configured data directory, the source-of-truth background document that anchors the
entire pipeline against hallucination.
...
| File | Description |
|---|---|
| `BACKGROUND_PATH` | Confirmed candidate background, with `**Verified:**` / `**Aspirational:**` markers where the distinction matters |
