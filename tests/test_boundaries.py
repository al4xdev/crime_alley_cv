...
def run_hook(
    ...
    return subprocess.run(args, capture_output=True, text=True, env=current_env, check=False)
...
def test_harvey_vera(mock_repo_structure):
    ...
    bg.write_text(
        "# Candidate Background Profile\n\nExperience: Senior Platform Engineer for Generative AI systems. Extended experience with Python, Cloud platforms, and Large Language Model fine-tuning techniques.",
        encoding="utf-8",
    )
...
def test_harvey_setup(mock_repo_structure):
    ...
    res_pre_fail = run_hook(
        "harvey_setup.fish", "--pre", env={"MAX_LOOPS": "3", "MIN_FIT_SCORE": "80"}
    )
    ...
    res_pre_pass = run_hook(
        "harvey_setup.fish",
        "--pre",
        env={"MAX_LOOPS": "3", "MIN_FIT_SCORE": "80", "KAREN_READS_BACKGROUND": "no"},
    )
    ...
    (session_dir / "anti_karen" / "who_are_u.md").write_text(
        "# Experience Details", encoding="utf-8"
    )

    try:
        # Post should pass with KAREN_READS_BACKGROUND=no
        res_post_pass = run_hook(
            "harvey_setup.fish", "--post", session_id, env={"KAREN_READS_BACKGROUND": "no"}
        )
        assert res_post_pass.returncode == 0

        # Fails if KAREN_READS_BACKGROUND=no but file leaked to docs/
        (session_dir / "docs" / "who_are_u.md").write_text("# Leaked", encoding="utf-8")
        res_post_fail_leak = run_hook(
            "harvey_setup.fish", "--post", session_id, env={"KAREN_READS_BACKGROUND": "no"}
        )
...
def test_bill_harvey(mock_repo_structure):
    ...
    checkpoint_file.write_text(
        '{"current_loop": 0, "fit_score": null, "session_id": "prev"}', encoding="utf-8"
    )
...
def test_gatekeeper_donna(mock_repo_structure):
    ...
    try:
        ...
        plan.write_text(
            "# Career Action Plan\n\n- Fix technology gaps in Python.\n- Run public open source projects.\n- Build Generative AI agents with robust sandboxing and deterministic contracts to prevent context fraud.",
            encoding="utf-8",
        )
